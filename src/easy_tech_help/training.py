...
from easy_tech_help.runtime import ADAPTER_DIR, MODEL_DIR, load_base, select_device


def encode_example(row, tokenizer) -> dict:
    ...
    return {"ids": full, "start": len(prefix), "id": row.id}
...
def train(args) -> dict:
    ...
    train_rows = [encode_example(r, tokenizer) for r in dataset["train"]]
    ...
    manifest = {
        ...
        "loss": "completion-only causal cross entropy; token-weighted accumulation",
        "selection": "lowest validation loss; test never used for training/selection",
    }
    ...
    for epoch in range(args.epochs):
        ...
        for offset in range(0, len(order), args.accumulation):
            ...
            tokens = sum(len(r["ids"]) - r["start"] for r in group)
