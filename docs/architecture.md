...
`rag.py` implements retrieval-augmented explanation. It maps `alert` to knowledge category `popup` and expands the original input with terms for observed signals. `retrieval.py` filters documents by category and weighted title/tag/body keywords, then ranks literal sentence-aware chunks with BM25 (k1=1.5, b=0.75) plus 0.05 times the document score. It groups adjacent short paragraphs and applies simple English plural normalization to search terms. Chunk IDs contain document ID, ordinal and content hash. The target size is 75 words; a longer sentence remains intact to retain conditions and consequences. The explanation path retrieves up to 18 candidates, at most six per document; standalone search retains its three/two defaults. Topic/context gates score literal sentence units and select one eligible reference, supplying up to three units to the explanation prompt: the small model mixed source facts when supplied with several excerpts but asked to cite only one. Other candidates remain in the trace and fallback reference display. The ten-document corpus is rebuilt in memory for each request; there is no persistent vector index or online fetch.
...
`observation_review.py` and `input_signals.py` apply auditable English application corrections. They distinguish code notifications from disclosure requests, reported alert/Settings context from ordinary messages, gift-card redemption numbers from account credentials, and informational URLs from link action requests. Literal cues supplement code/payment/remote-access, URL/phone and Wi-Fi states. An exact “No Internet Connection” quote cannot support a connected or unsecured-network signal; those need independent joining/security evidence. Clause-local negation and historical/educational exclusions apply. Contradictory current Wi-Fi states return `unknown/contradictory_input`; analyzer commands without phone context return `unknown/unsupported`.

Invalid extraction remains rejected except for narrowly supported evidence recovery: a unique case-insensitive input match can restore the original case, or a model's code/connection evidence can be replaced with an independently located literal code-request/connected-state cue. The entire corrected object must then pass the original strict schema and exact-span validator. Unsupported signals, malformed JSON and other ungrounded evidence remain invalid. Raw generation, original observation and correction reasons stay in the diagnostic result. Recovery is application behavior, not improved model generation.
...
| `observation_review.py`, `input_signals.py`, `review_evaluation.py` | Literal context cues, application corrections and replay of recorded raw predictions |
...
| `rag.py`, `reference_relevance.py`, `rag_evaluation.py` | Extractive RAG, topic/context gates, citation validation and development checks |
| `product_evaluation.py`, `verification_evaluation.py`, `data/evaluation/` | Full pipeline regression and authored verification, retaining failed checks and traces |
...
See [quality improvement results](../results/quality_improvement.md) for the latest full pipeline, additional authored inputs and browser validation. The [initial audit](../results/full_evaluation.md) is preserved as the before-fix record. No model retraining occurred during these application fixes. The historical RAG checker required a particular past/present passage for public Wi-Fi. Its prior failure remains in the 70-case report. A subsequent checker correction requires both halves if either temporal marker is quoted, while permitting a different complete HTTPS passage; four regression tests cover this distinction. Input cases, expected statuses and expected source documents are unchanged.
